...
import { Box, Flex, Spinner, Center, Text, Image, Button, useDisclosure } from "@chakra-ui/react";
...
const TaskCalendarPage = () => {
  ...
  const handleEventDrop = (taskId: string, start: Date, end: Date) => {
    updateTaskMutation.mutate(
      { id: taskId, updates: { startDate: start.toISOString(), endDate: end.toISOString() } },
      {
  ...
  if (isError) {
    return (
      <Center h="100vh" flexDirection="column" gap={4}>
        <Text fontSize="xl" color="red.500">Failed to load tasks</Text>
  ...
  return (
    <Box p={{ base: 4, md: 8 }} bg="gray.50" _dark={{ bg: "gray.900" }} minH="100vh" w="100%">
      <CalendarHeader
      ...
      <Box bg="white" _dark={{ bg: "gray.800" }} p={6} borderRadius="2xl" boxShadow="sm">
        ...
        {filteredTasks.length === 0 && !isLoading ? (
          <Center py={20} flexDirection="column">
            ...
            <Text fontSize="xl" fontWeight="600" mb={2}>No Tasks Scheduled</Text>
            <Text color="gray.500" mb={6}>You don't have any legal tasks for this period.</Text>
            <Button colorScheme="blue" onClick={() => setIsModalOpen(true)}>Create First Task</Button>
          </Center>
        ) : (
