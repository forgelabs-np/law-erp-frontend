...
const generateDummyTasks = (): Task[] => {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    const startDate = setMinutes(
      setHours(addDays(baseDate, daysOffset), hour),
      0
    );
    ...
    switch (taskType) {
      case "Court Hearing":
        title = "Hearing for Case " + getRandomInt(1000, 9999);
        break;
      case "Client Meeting":
        title = "Meeting with " + getRandomItem(clients);
        break;
      case "Legal Research":
        title = "Research precedents for " + getRandomItem(clients);
        break;
      case "Filing":
        title = "File motions to dismiss";
        break;
      case "Review":
        title = "Review contracts for merger";
        break;
      case "Internal Task":
    ...
    tasks.push({
      ...
      title,
      description:
        "This is a detailed description for the legal task. It requires careful attention.",
      taskType,
